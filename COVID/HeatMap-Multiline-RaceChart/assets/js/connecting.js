...
d3.json("p2p.json", function(data) {
  var dataset= data;
    console.log(dataset);
...
.domain(["Deceased", "Hospitalized", "Recovered","Infected", ""])
...
//   var k = Math.sqrt(nodes.length / (width * height));
  ...
  force
      .nodes(nodes)
      .links(links)
      // .size([nodes.length *40, nodes.length*40])
  ...
  .attr("height", height)
  .attr("width", width)
  // .attr("height", nodes.length *40)
  // .attr("width", nodes.length *40)
